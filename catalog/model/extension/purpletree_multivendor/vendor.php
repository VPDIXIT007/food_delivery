...
class ModelExtensionPurpletreeMultivendorVendor extends Model{
	...
	////end quick order ////
		...
		public function getSlider($store_id){
			$query = $this->db->query("SELECT image FROM " . DB_PREFIX . "slider_table WHERE seller_id = '". (int)$store_id ."'");
			return $query->rows;
		}
		public function getStore($store_id){
			trigger_error($store_id);
		...
		public function getSlides($store_id) {
			
			//$seller_id = $this->customer->getId();
			
			$query = $this->db->query("SELECT pvs.* FROM " . DB_PREFIX . "slider_table pvs pvs.seller_id = '".(int)$store_id."' ORDER BY sort_order ASC");
		...
		public function getStoreDetail($customer_id){
			...
			$query = $this->db->query("SELECT pi.* FROM " . DB_PREFIX . "slider_table pi WHERE  pi.seller_id = '".(int)$store_id."' ORDER BY sort_order ASC");
			
			return $query->rows;
		}
		public function editStoreImage($store_id){	
			...
			public function getNoOfSlides($store_id){
				$seller_id = $this->customer->getId();
			$query=$this->db->query("SELECT pvp.no_of_slides FROM " . DB_PREFIX . "purpletree_vendor_plan pvp LEFT JOIN " . DB_PREFIX . "purpletree_vendor_seller_plan pvsp ON (pvp.plan_id=pvsp.plan_id) WHERE pvsp.seller_id='".(int) $seller_id."' AND pvsp.status=1");
			if($query->num_rows){	
				return $query->row;
				} else {
				return NULL;	
