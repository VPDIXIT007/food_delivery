...
class ModelExtensionPurpletreeMultivendorSellerproduct extends Model{
	   ////// quick order //////
		...
		public function getSubcategory($parent_id) {		
			$query = $this->db->query("SELECT DISTINCT c.category_id,cd.name,c.catimg  FROM " . DB_PREFIX . "category c LEFT JOIN " . DB_PREFIX . "category_description cd ON (c.category_id = cd.category_id AND cd.language_id = '" . (int)$this->config->get('config_language_id')."')WHERE c.parent_id = '" . (int)$parent_id . "'"); 
			return $query->rows;
		}
	   ...
	   ////  end quick order ////
		...
		$seller_id = $this->model_extension_purpletree_multivendor_vendor->getStore($store_id);
			$sql .= " AND pvp.seller_id = '".$seller_id['seller_id']."'";
			$query = $this->db->query($sql);
			if($query->num_rows>0){
				return $query->rows;
				} else {
