...
class ModelExtensionPurpletreeMultivendorDownloads extends Model {
		...
		public function addDownload($data,$seller_id = null) {
			$this->db->query("INSERT INTO " . DB_PREFIX . "download SET filename = '" . $this->db->escape($data['filename']) . "', mask = '" . $this->db->escape($data['mask']) . "', date_added = NOW(), seller_id = '" . $this->db->escape($seller_id) . "'");
