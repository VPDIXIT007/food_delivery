...
class ModelCatalogProduct extends Model {
	...
	public function getProducts($data = array(),$storeseller_id = 0) {
		...
		if($storeseller_id > 0) {
			...
			$sql .= " LEFT JOIN oc_purpletree_vendor_products pvp on (p.product_id = pvp.product_id) ";
		}
		$sql .= " LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id) LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (p.product_id = p2s.product_id) WHERE pd.language_id = '" . (int)$this->config->get('config_language_id') . "' AND p.status = '1' AND p.date_available <= NOW() AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "'";
		if($storeseller_id > 0) {
		$sellerid = $this->db->query("SELECT seller_id FROM ". DB_PREFIX ."purpletree_vendor_stores WHERE id = ". (int)$storeseller_id ."")->row;
		$sql .= " AND pvp.seller_id = '". (int)$sellerid['seller_id'] ."' ";
		}
		if (!empty($data['filter_category_id'])) {
			if (!empty($data['filter_sub_category'])) {
