// Image Manager
    ...
    if(side && side == 'rtl') {
		...
		$('#pts-button-image').on('click', function() {
			...
			var seller_name = ($('input[name=\'seller_name\']').val());
			$.ajax({
				url: 'index.php?route=extension/common/filemanager&target=' + $element.parent().find('input').attr('id') + '&thumb=' + $element.attr('id')+'&seller_id=' + seller_id +'&seller_name=' + seller_name +'&banneredit=yes',
				dataType: 'html',
				beforeSend: function() {
					$button.prop('disabled', true);
					if ($icon.length) {
