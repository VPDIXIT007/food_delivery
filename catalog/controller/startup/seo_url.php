...
class ControllerStartupSeoUrl extends Controller {
		...
		foreach ($data as $key => $value) {
			if (isset($data['route'])) {
				if (($data['route'] == 'product/product' && $key == 'product_id') || (($data['route'] == 'product/manufacturer/info' || $data['route'] == 'product/product') && $key == 'manufacturer_id') ) {
					if($data['seller_store_id'] > 0) {
						if($data['route'] == 'product/product') {
						$query1 = $this->db->query("SELECT * FROM " . DB_PREFIX . "seo_url WHERE `query` = '" . $this->db->escape('seller_store_id' . '=' . (int)$data['seller_store_id']) . "' AND store_id = '" . (int)$this->config->get('config_store_id') . "'");
	...
	if (isset($data['route'])) {
				if ($data['route'] == 'checkout/checkout' || $data['route'] == 'checkout/cart') {
					if($data['seller_store_id'] > 0) {
					...
					$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "seo_url WHERE `query` = '" . $this->db->escape($data['route']) . "' AND store_id = '" . (int)$this->config->get('config_store_id') . "'");
