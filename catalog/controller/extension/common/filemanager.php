...
class ControllerExtensionCommonFileManager extends Controller {
		public function index() { 
			...
			if (isset($this->request->get['banneredit'])) {
				$data['banneredit'] = 'yes';
			} 
			...
			if (!$json) {
				...
				foreach ($files as $file) {
					if (is_file($file['tmp_name'])) {
						...
						if (isset($this->request->get['banneredit']) && $this->request->get['banneredit'] == 'yes') {
							
						$imagesize = getimagesize($file['tmp_name']);
						
						$imageWidth = $imagesize[0]; //Contains the Width of the Image
						$imageHeight = $imagesize[1]; //Contains the Height of the Image
						$maxwidth = $this->config->get('theme_mahardhi_image_bannerslider_width');
						$maxheight = $this->config->get('theme_mahardhi_image_bannerslider_height');
					
						if($imageWidth < $maxwidth || $imageHeight < $maxheight)  
						{ 
						
						$json['error'] = $this->language->get('error_filesize'); //Your error
						}
						}
