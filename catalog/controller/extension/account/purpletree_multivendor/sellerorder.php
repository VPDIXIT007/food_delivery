...
class ControllerExtensionAccountPurpletreeMultivendorSellerorder extends Controller{
		...
		public function seller_order_info(){
			...
			if ($order_info) { 
				...
				foreach ($products as $product) {
					...
					'name'    	 	   => $product['name'],
					'pcomment'    	 	   => $product['product_comments'],
					'model'    		   => $product['model'],
					'option'   		   => $option_data,
		...
		public function history() {
			...
			//}
				// PUSH NOTIFICATION
			$push = $this->sendNotification($order_id,$this->request->post['order_status_id']);
		...
		function sendNotification($order_id,$order_status_id){
			// GET SESSION ID AND TOKEN
		
			$sessionorder = $this->db->query("SELECT session_id from ". DB_PREFIX ."session WHERE data LIKE '%". $order_id ."%'")->row;
    ...
    $fields=array(
        ...
        "notification"=>array(
            "body"=>'Your order #'. $order_id .' is '. $statusname['name'] .'',//$_REQUEST['message'],
   ...
   // print_r($result);
	...
	return $result;
		...
		public function changetable(){
			...
			$tablenum = $this->request->post['tablenum'];
			$sellerid = $this->customer->getId();
			
		$json = array();
			//check if table exists for this seller_href
			$table = $this->db->query('SELECT * FROM '. DB_PREFIX .'table_manger WHERE table_no = '. $tablenum .' AND seller_id = '. $sellerid .'')->row;
			if(isset($table)) {
				
				$updateorder = $this->db->query('UPDATE '. DB_PREFIX .'purpletree_vendor_orders SET table_id='. (int)$table['id'] .' WHERE order_id='. (int)$orderid .' ');
