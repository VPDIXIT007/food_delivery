...
class ControllerExtensionAccountPurpletreeMultivendorSellerstore extends Controller{
	...
	public function index(){
		...
		$data['entry_additional_slide'] = $this->language->get('entry_additional_slide');
		...
		if (isset($store_id) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
			$data['no_of_slides'] = $this->model_extension_purpletree_multivendor_vendor->getNoOfSlides($store_id);
		...
		// edit abdo store banner
		//$slider = array();
		/*
		if(isset($seller_info['store_banner'])) {
		if(!empty($seller_info['store_banner'])) {
		$slider = explode('###',$seller_info['store_banner']);
		} else {$slider = '';}
		if(isset($slider[0])) {$seller_info['store_banner'] = $slider[0];} else {$seller_info['store_banner'] = '';}
		if(isset($slider[1])) {$seller_info['store_banner2'] = $slider[1];} else {$seller_info['store_banner2'] = '';}
		if(isset($slider[2])) {$seller_info['store_banner3'] = $slider[2];} else {$seller_info['store_banner3'] = '';}
		}*/
		// end abdo
       ...
       if (isset($this->request->post['store_shipping_order_type'])) {
		...
		// Images
			if (isset($this->request->post['store_banner'])) {
				$store_banner = $this->request->post['store_banner'];
					} elseif (!empty($seller_info)) {
				$store_banner = $this->model_extension_purpletree_multivendor_vendor->getStoreImages($store_id);
				} else {
				$store_banner = array();
			}
			
			$data['store_banner'] = array();
			
			foreach ($store_banner as $slide) {
		...
		// abdo slider edit 
		/*
		if (isset($this->request->post['store_banner2'])) {
			$data['store_banner2'] = $this->request->post['store_banner2'];
		} elseif (!empty($seller_info)) {
			$data['store_banner2'] = $seller_info['store_banner2'];
		} else {
			$data['store_banner2'] = '';
		}	
		if (isset($this->request->post['store_banner3'])) {
			$data['store_banner3'] = $this->request->post['store_banner3'];
		} elseif (!empty($seller_info)) {
			$data['store_banner3'] = $seller_info['store_banner3'];
		} else {
			$data['store_banner3'] = '';
		}	
		if (isset($this->request->post['store_banner2']) && is_file(DIR_IMAGE . $this->request->post['store_banner2'])) {
			$data['banner_thumb2'] = $this->model_tool_image->resize($this->request->post['store_banner2'], 100, 100);
		} elseif (!empty($seller_info) && is_file(DIR_IMAGE . $seller_info['store_banner2'])) {
			$data['banner_thumb2'] = $this->model_tool_image->resize($seller_info['store_banner2'], 100, 100);
		} else {
			$data['banner_thumb2'] = $this->model_tool_image->resize('catalog/purpletree_banner.jpg', 100, 100);
		}	
		if (isset($this->request->post['store_banner3']) && is_file(DIR_IMAGE . $this->request->post['store_banner3'])) {
			$data['banner_thumb3'] = $this->model_tool_image->resize($this->request->post['store_banner3'], 100, 100);
		} elseif (!empty($seller_info) && is_file(DIR_IMAGE . $seller_info['store_banner3'])) {
			$data['banner_thumb3'] = $this->model_tool_image->resize($seller_info['store_banner3'], 100, 100);
		} else {
			$data['banner_thumb3'] = $this->model_tool_image->resize('catalog/purpletree_banner.jpg', 100, 100);
		}	*/
		// abdo end
 ...
 $this->document->addScriptpts('catalog/view/javascript/purpletree/summernote/opencart.js'); 
    ...
    $installed_modules = $this->model_setting_extension->getExtensions('module');
		...
		$this->load->model('extension/purpletree_multivendor/vendor');
			...
			$data['seller_store_id']=$seller_Store;
		...
		$pdfmenu = null;
			$abdo = $this->db->query("SELECT plan_id FROM ". DB_PREFIX ."purpletree_vendor_plan_invoice WHERE seller_id = '". $seller_Store['seller_id'] ."'")->row;
		if ($abdo['plan_id'] == 3) {
			//$pdfdownload = $this->db->query("SELECT mask FROM ". DB_PREFIX ."download WHERE seller_id = '". $seller_Store['seller_id'] ."'")->row;
			$pdfmenu = $this->db->query("SELECT mask FROM ". DB_PREFIX ."download WHERE seller_id = '". $this->request->get['seller_store_id'] ."'")->row;
		}
			trigger_error(print_r($this->request->get['seller_store_id'] ,true));
		...
		$category_p=0;
		if(!empty($this->request->get['p_url']))
		{
			$category_p=$this->request->get['p_url'];
		}
		...
		if(isset($this->request->get['table_id'])) {
				...
				$decryption_key = "encryptedtableid"; 
  
				// Use openssl_decrypt() function to decrypt the data 
				$decryption= openssl_decrypt($this->request->get['table_id'], $ciphering,  
        $decryption_key, $options, $decryption_iv); 
		//trigger_error($decryption);
			$this->session->data['table_id'] = $decryption;
		}
		if(isset($this->request->get['seller_store_id'])){
		...
		$url = '';
			...
			if(isset($this->request->get['seller_store_id'])){
				$url .= '&seller_store_id=' . $this->request->get['seller_store_id'];
		...
		if($category_p > 1) {
			if (preg_match('/^[^\W_]+_[^\W_]+$/', $category_p)) {
			$catscore = explode('_',$category_p);
			//print_r($catscore);
			$category_p = $catscore[1];
				}
			$categoryMenu2 =$this->model_extension_purpletree_multivendor_sellerproduct->getSubcategory($category_p);
				$slidercat=array();
			if(!empty($categoryMenu2)){
					foreach($categoryMenu2 as $category_key2=>$category_value2){
						$categoryMenuProduct2 =$this->model_extension_purpletree_multivendor_sellerproduct->categoryMenuProduct($this->request->get['seller_store_id'],$category_value2['category_id']);
						$countproducts = count($categoryMenuProduct2);
						if($countproducts > 0) {
						if($category_value2['category_id']){
						$data['slidercat'][]=array(
						'url'=>$this->url->link('extension/account/purpletree_multivendor/sellerstore/storeview', '&seller_store_id='.$this->request->get['seller_store_id'].'&p_url='.$category_value2['category_id'], true),
						'name'=>$category_value2['name'],
						'img'=> $category_value2['catimg']
		...
		} else {$data['slidercat'] = $data['storeMenu'];}
		trigger_error(print_r($data['slidercat'],true));
                ...
                $data['show_seller_address'] = $this->config->get('module_purpletree_multivendor_show_seller_address');
					...
					$google_map = '';
				       ...
				       $data['seller_products'][] = array('seller_name'  => isset($seller_detail['seller_name'])?$seller_detail['seller_name']:'',
				        ...
				        'quick_order'       => $this->url->link('extension/account/purpletree_multivendor/quick_order', '&product_id='.$seller_product['product_id'].'&seller_store_id='.$sellerstore.''.$url, true),
						'href'  => $this->url->link('product/product', 'product_id=' . $seller_product['product_id'].'&seller_store_id='.$sellerstore.''.$conditu,true),
		...
		if(isset($pdfmenu)) {
		$path = 'image/catalog/sellermenu/seller_'.$seller_id.'/'.$pdfmenu['mask'];
		$path_info = pathinfo($path);
		if($path_info['extension'] == 'pdf') { 	$data['pdfmenus'] = $path;} else {	$data['imgmenu'] = $path;} 
		//$data['pdfmenus'] = $pdfmenu['mask'];
		$this->response->setOutput($this->load->view('account/purpletree_multivendor/menuview', $data));
		} else {
	...
	public function sellerreview() { 
		...
		if (isset($this->request->get['seller_store_id'])) {
			$seller_store_id = (int)$this->request->get['seller_store_id'];
		} else {
			$seller_store_id = 0;
		}
		...
		$data['breadcrumbs'][] = array(
			'text' => $this->language->get('text_home'),
			'href' => $this->url->link('extension/account/purpletree_multivendor/sellerstore/storeview', 'seller_store_id='.$seller_store_id,true)
		);	
	...
	public function productreview() { 
		...
		$data['customer_id'] = $this->customer->getId();
			$data['enabledreview'] = $this->url->link('extension/account/purpletree_multivendor/sellerstore/enabledreview', '', true);
			$data['disabledreview'] = $this->url->link('extension/account/purpletree_multivendor/sellerstore/disabledreview', '', true);

		$this->load->language('purpletree_multivendor/productreview');
		
		$this->load->model('extension/purpletree_multivendor/productreview');

		if ($this->request->server['REQUEST_METHOD'] == 'POST' && $this->validateReview()) {
			
			$this->model_extension_purpletree_multivendor_productreview->addReview($this->request->post);

			$this->session->data['success'] = $this->language->get('text_success');

			$this->response->redirect($this->url->link('extension/account/purpletree_multivendor/sellerstore/productreview&seller_id='.$this->request->post['seller_id'],'',true));
		}
		...
		$data['entry_status'] = $this->language->get('text_status');
		$data['text_all'] = $this->language->get('text_all');
		$data['text_enabled'] = $this->language->get('text_enabled');
		$data['text_disabled'] = $this->language->get('text_disabled');
		...
		if(isset($this->request->get['seller_id'])){
			$data['breadcrumbs'][] = array(
				'text' => $this->language->get('text_storereview'),
				'href' => $this->url->link('extension/account/purpletree_multivendor/sellerstore/productreview&seller_id='.$this->request->get['seller_id'], '', true)
			);
			$data['seller_id'] = $seller_id;
			$this->load->model('extension/purpletree_multivendor/productreview');
		if(!$this->model_extension_purpletree_multivendor_sellerreview->canReview($datasend = array('seller_id' =>$seller_id,'customer_id' =>$data['customer_id']))) {
			...
			$data['action'] = $this->url->link('extension/account/purpletree_multivendor/sellerstore/productreview&seller_id='.$data['seller_id'],'',true);
			...
			$filter_data = array(
				...
				'product_id' 		=>$this->request->get['filter_name'],
				'customer_id'		=> $data['customer_id']
			);
			
			$review_total = $this->model_extension_purpletree_multivendor_productreview->getTotalSellerReview($filter_data);
		 ...
		 if ($direction=='rtl'){
			...
			$seller_id = $this->customer->isSeller();
			
			$data['breadcrumbs'][] = array(
				'text' => $this->language->get('text_storereview'),
				'href' => $this->url->link('extension/account/purpletree_multivendor/sellerstore/productreview', '', true)
			);
			$filter_data = array(
				'start'              => ($page - 1) * $limit,
				'limit'              => $limit,
			...
			$results = $this->model_extension_purpletree_multivendor_productreview->getSellerReview($filter_data);
			...
			if ($results) {
				foreach($results as $result){
					$data['reviews'][] = array(
						...
						'status'     => $result['status'],
						'customer_name'     => $result['customer_name'],
						'review_title'     => $result['author'],
						'review_description'       => nl2br($result['text']),
						'rating'     => (int)$result['rating'],
						'status'     => (($result['status'])?$this->language->get('text_approved'):$this->language->get('text_notapproved')),
				...
				$reviewed_products = $this->model_extension_purpletree_multivendor_productreview->getReviewProduct($data['customer_id']);
				
				$data['pruductlists'] = array();
				if ($reviewed_products) {
				 foreach ($reviewed_products as $products) {
					
					$data['pruductlists'][] = array(
					'productid' => $products['product_id'],
					'name'       => strip_tags(html_entity_decode($products['name'], ENT_QUOTES, 'UTF-8'))
					);
				   }  
				}
			...
			$pagination->page = $page;
			$pagination->limit = $limit;
			$pagination->url = $this->url->link('extension/account/purpletree_multivendor/sellerstore/productreview', 'page={page}',true);
	...
	public function enabledreview(){
	
			
			if (!$this->customer->isLogged()) {
				$this->session->data['redirect'] = $this->url->link('extension/account/purpletree_multivendor/productreview', '', true);
			...
			if(!$this->customer->validateSeller()) {
				$this->load->language('purpletree_multivendor/ptsmultivendor');
				$this->session->data['error_warning'] = $this->language->get('error_license');			
				$this->response->redirect($this->url->link('extension/account/purpletree_multivendor/sellerstore/productreview', '', true));
			}
			$this->load->language('purpletree_multivendor/productreview');
			...
			$this->load->model('extension/purpletree_multivendor/productreview');
				//trigger_error(print_r($this->request->post['selected'],true));
			if (isset($this->request->post['selected'])) {
			
				foreach ($this->request->post['selected'] as $table_id) {
					$this->model_extension_purpletree_multivendor_productreview->enabledreview($table_id);
				}
		...
		public function disabledreview(){
			...
			if (isset($this->request->post['selected'])) {
				...
				foreach ($this->request->post['selected'] as $table_id) {
					$this->model_extension_purpletree_multivendor_productreview->disabledreview($table_id);
				}
