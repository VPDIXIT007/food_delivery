...
class ControllerExtensionAccountPurpletreeMultivendorDownloads extends Controller {
		...
		public function add() {
			...
			if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
				$this->model_extension_purpletree_multivendor_downloads->addDownload($this->request->post,$seller_id);
		...
		public function upload() {
			...
			if (!$json) {
				...
				$store_detail = $this->customer->isSeller();
				$seller_id = $store_detail['id'];
				if (!file_exists( DIR_IMAGE .'/catalog/sellermenu/seller_'.$seller_id.'/')) {
				mkdir( DIR_IMAGE .'/catalog/sellermenu/seller_'.$seller_id.'/', 0777, true);
				}
				move_uploaded_file($this->request->files['file']['tmp_name'], DIR_IMAGE .'/catalog/sellermenu/seller_'.$seller_id.'/'. $file2);
				
				copy(DIR_IMAGE.'/catalog/sellermenu/seller_'.$seller_id.'/'. $file2, DIR_DOWNLOAD . $file);
				$json['filename'] = $file;
				$json['mask'] = $filename;
