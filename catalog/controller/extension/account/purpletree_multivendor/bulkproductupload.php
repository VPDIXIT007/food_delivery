		...
		public function upload(){
			...
			if (($this->request->server['REQUEST_METHOD'] == 'POST')) {
				...
				if($_FILES["import"]['tmp_name'] != '') {	
					$mimes = array('application/octet-stream','application/vnd.ms-excel','text/xls','text/xlsx','application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
					trigger_error($_FILES["import"]["type"]);
											...
											try {
												if(is_numeric($data['product_id']) && $data['product_id']!='' && !is_string($data['product_id']))
												{
													...
													$addProduct_action=strtoupper(trim($filter_data['add_product']));
														//$addProduct_action = 'YES';
													if($addProduct_action){
														// Subscription
														if($this->config->get('module_purpletree_multivendor_subscription_plans')){
