...
class ControllerExtensionPurpletreeMultivendorSubscriptionplan extends Controller {
		...
		public function edit() {
			...
			if (($this->request->server['REQUEST_METHOD'] == 'POST')  && $this->validateForm() ) {
				...
				if(!isset($this->request->post['enable_theme_setting'])){
					$this->request->post['enable_theme_setting']=0;	
				}
		...
		protected function getForm() {
			...
			// Abdo edit
			if (isset($this->error['no_of_slides'])) {
				$data['errorno_of_slides'] = $this->error['no_of_slides'];
				} else {
				$data['error_no_of_slides'] = '';
			}
			if (isset($this->error['no_of_tables'])) {
				$data['error_no_of_tables'] = $this->error['no_of_tables'];
				} else {
				$data['error_no_of_tables'] = '';
			}
			// Abdo End
			...
			// Abdo edit
				if (isset($this->request->post['no_of_slides'])) {
				$data['no_of_slides'] = $this->request->post['no_of_slides'];
				} elseif (!empty($plan_info)) {
				$data['no_of_slides'] = $plan_info['no_of_slides'];
				} else {
				$data['no_of_slides'] = '';
			}
				if (isset($this->request->post['no_of_tables'])) {
				$data['no_of_tables'] = $this->request->post['no_of_tables'];
				} elseif (!empty($plan_info)) {
				$data['no_of_tables'] = $plan_info['no_of_tables'];
				} else {
				$data['no_of_tables'] = '';
			}
			// Abdo ENd
			...
			if (isset($this->request->post['enable_theme_setting'])) {
				$data['enable_theme_setting'] = $this->request->post['enable_theme_setting'];
				} elseif (!empty($plan_info)) {
				$data['enable_theme_setting'] = $plan_info['enable_theme_setting'];
				} else {
				$data['enable_theme_setting'] = '';
			}
		...
		protected function validateForm() {
			...
			// abdo edit
				if( ! filter_var($this->request->post['no_of_slides'], FILTER_VALIDATE_INT) && $this->request->post['no_of_slides'] != '0') {
				$this->error['no_of_slides']= "Must be numeric value";
			}
				if( ! filter_var($this->request->post['no_of_tables'], FILTER_VALIDATE_INT) && $this->request->post['no_of_tables'] != '0') {
				$this->error['no_of_tables']= "Must be numeric value";
			}
			// abdo end
